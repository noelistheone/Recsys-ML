...
class PT4Rec_Adv(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(PT4Rec_Adv, self).__init__(conf, training_set, test_set)

        args = OptionConf(self.config['PT4Rec_Adv'])
        ...
        self.lambda_rev = float(args['-lambda_rev']) if args.contain('-lambda_rev') else 1.0
        self.adv_weight = float(args['-adv_weight']) if args.contain('-adv_weight') else 0.1
        ...
        if self.user_prompt_num != 0:         
            ...
            self.discriminator = NoiseDiscriminator(prompt_size).cuda()
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                prompted_user_emb, prompted_item_emb, prompt = self.generate_prompts(user_emb, item_emb)
                ...
                batch_user_emb, pos_item_emb, neg_item_emb = rec_user_emb[user_idx], rec_item_emb[pos_idx], rec_item_emb[neg_idx]
                rec_loss = bpr_loss(batch_user_emb, pos_item_emb, neg_item_emb)

                # 第二阶段 不继续训练对比学习权重
                batch_loss = rec_loss + l2_reg_loss(self.reg, batch_user_emb, pos_item_emb)
                
                # ==== Adversarial Denoising with GRL ====
                if prompt is not None and hasattr(self, 'discriminator'):
                    batch_prompt = prompt[user_idx]
                    
                    # 伪造 noise_labels (由于原框架未提供，使用随机标签模拟；实际中应使用样本侧信息/特征作为依据)
                    # 例如，标记某些长尾物品或具有特定行为模式的用户为噪声 (1) 
                    noise_labels = torch.randint(0, 2, (batch_prompt.size(0),), dtype=torch.float32).cuda()
                    
                    # Gradient Reversal Layer
                    grl_prompt = GradientReversalLayer.apply(batch_prompt, self.lambda_rev)
                    
                    # Generator tries to trick Discriminator; Discriminator tries to predict true noise.
                    noise_logits = self.discriminator(grl_prompt).squeeze(-1)
                    
                    adv_loss = self.bce_loss(noise_logits, noise_labels)
    ...
    def generate_prompts(self, user_emb, item_emb):
        ...
        return prompted_user_emb, item_emb, prompt
